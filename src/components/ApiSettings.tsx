...
const COMMON_KEYS: KeySpec[] = [
  { field: 'zhenzhenApiKey', label: '雪糕工坊 API Key', desc: '· 通用后备 · 用于图像/视频/音频生成', bullet: 'bg-amber-400' },
...
const SETTINGS_BACKUP_SCHEMA = 'red-canvas-settings';
    ...
    >
      ...
      >
        ...
        <div className="t8-api-settings-body p-5 space-y-5 overflow-y-auto">
          ...
          {renderKey(COMMON_KEYS[2], { baseUrlNote: `Base URL 锁定: ${FIXED_ZHENZHEN_BASE} (与雪糕同地址, Key 独立)` })}
          ...
          <div className="t8-api-settings-divider pt-3 border-t">
            ...
            {!classifiedOpen && (
              <div className={`text-[11px] mt-2 ${hintCls}`}>
                不必担心：<b>未填项会自动 fallback 到雪糕工坊通用 Key</b>，新手可直接保存忽略此区块。
              </div>
            )}
            {classifiedOpen && (
              <div className="mt-3">
                <div className={`text-[11px] ${hintCls} mb-3`}>
                  为不同模型系列单独配置 Key；<b>未填则自动 fallback 到雪糕工坊通用 Key</b>。后端会根据调用的模型名/路由自动选择。
                </div>
          ...
          <div className="t8-api-settings-divider pt-3 border-t">
            ...
            {!advancedOpen && (
              <div className={`text-[11px] mt-2 ${hintCls}`}>
                未配置或未启用时不会影响雪糕工坊、RunningHub、LLM 独立 Key 等主流程。
              </div>
            )}
            {advancedOpen && (
